...
public class LocalSaveHookForWavResponse extends AbstractSaveHook {
	public static Logger LOG = LoggerFactory.getLogger(LocalSaveHookForWavResponse.class);
	...
	public LocalSaveHookForWavResponse(File absPathToWavDir){
		...
		LOG.info("Wav dir was set to '" + wavDir.getAbsolutePath() + "'");
		
		
		if(this.wavDir.exists() || this.wavDir.isDirectory()){
			// -- good, existing dir!
		}else{
			LOG.info("Creating text wav dir  '" + this.wavDir.getAbsolutePath() + "'");
			this.wavDir.mkdirs();
		}
	}
	...
	public void save(Response resp){
		...
		if(content instanceof ResponseContentAudio){
			...
			try{
				LOG.info("Saving wav file file '" + wavPath.getAbsolutePath() + "'");
				FileOutputStream fos = new FileOutputStream(wavPath);
				fos.write(bytes);
				fos.close();
			}catch(IOException e){
				LOG.info("Failed to write text transcript file '" + wavPath.getAbsolutePath() + "'");
			}
			...
			try {
				LOG.info("Creating text transcript file '" + trsPath.getAbsolutePath() + "'");
				PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(trsFileName, true)));
				writer.println(hdr.taskBrief);
				writer.flush();
				writer.close();
			} catch (IOException e) {
				LOG.info("Failed to write text transcript file '" + trsPath.getAbsolutePath() + "'");
			}
		}
	}
